...
public:
    ...
    explicit Slotmap() { clear(); }
     ...
     key_type    push_back(value_type const& newVal) {
        return push_back( value_type{ newVal } ); 
    }
     key_type    push_back(value_type&& newVal) {
    ...
    iterator cbegin() const { return data_.cbegin(); }

    iterator   end() { return data_.begin() + size_; }
    iterator  cend() const { return data_.cbegin() + size_; }
